...
train_control <- trainControl(method="repeatedcv", number=3, repeats=3,  summaryFunction = twoClassSummary, classProbs = TRUE, sampling = "up")
...
#Training Results
Final_all_models_tr_res <- bind_rows(res_rf, res_svr, res_svmL, res_glm, res_LRB, res_glmnet, res_knn, res_C5, res_lda, res_MLP, res_vgam, res_xgbDART, res_xgbT, res_nnet)

rownames(Final_all_models_tr_res) <- c("RF", "SVM-RBF", "SVM-L", "GLM", "Boosted_LR", "GlmNet", "KNN",  "C5-DT", "LDA", "MLP", "VGAM", "xgbDART", "xgbTree", "Neural_NET" )
